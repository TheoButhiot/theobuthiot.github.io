...
`,after:()=>{console.log("Component navigation mounted")}};var j=typeof globalThis<"u"?globalThis:typeof window<"u"?window:typeof global<"u"?global:typeof self<"u"?self:{},ce={exports:{}};(function(t,e){(function(n,r){t.exports=r()})(j,function(){var n=document,r=n.createTextNode.bind(n);function o(a,i,s){a.style.setProperty(i,s)}function c(a,i){return a.appendChild(i)}function u(a,i,s,f){var m=n.createElement("span");return i&&(m.className=i),s&&(!f&&m.setAttribute("data-"+i,s),m.textContent=s),a&&c(a,m)||m}function d(a,i){return a.getAttribute("data-"+i)}function v(a,i){return!a||a.length==0?[]:a.nodeName?[a]:[].slice.call(a[0].nodeName?a:(i||n).querySelectorAll(a))}function p(a){for(var i=[];a--;)i[a]=[];return i}function g(a,i){a&&a.some(i)}function O(a){return function(i){return a[i]}}function M(a,i,s){var f="--"+i,m=f+"-index";g(s,function(b,y){Array.isArray(b)?g(b,function(S){o(S,m,y)}):o(b,m,y)}),o(a,f+"-total",s.length)}var L={};function $(a,i,s){var f=s.indexOf(a);if(f==-1)s.unshift(a),g(L[a].depends,function(b){$(b,a,s)});else{var m=s.indexOf(i);s.splice(f,1),s.splice(m,0,a)}return s}function T(a,i,s,f){return{by:a,depends:i,key:s,split:f}}function I(a){return $(a,0,[]).map(O(L))}function h(a){L[a.by]=a}function A(a,i,s,f,m){a.normalize();var b=[],y=document.createDocumentFragment();f&&b.push(a.previousSibling);var S=[];return v(a.childNodes).some(function(x){if(x.tagName&&!x.hasChildNodes()){S.push(x);return}if(x.childNodes&&x.childNodes.length){S.push(x),b.push.apply(b,A(x,i,s,f,m));return}var H=x.wholeText||"",J=H.trim();J.length&&(H[0]===" "&&S.push(r(" ")),g(J.split(s),function(Me,Ae){Ae&&m&&S.push(u(y,"whitespace"," ",m));var K=u(y,i,Me);b.push(K),S.push(K)}),H[H.length-1]===" "&&S.push(r(" ")))}),g(S,function(x){c(y,x)}),a.innerHTML="",c(a,y),b}var N=0;function D(a,i){for(var s in i)a[s]=i[s];return a}var E="words",l=T(E,N,"word",function(a){return A(a,"word",/\s+/,0,1)}),k="chars",C=T(k,[E],"char",function(a,i,s){var f=[];return g(s[E],function(m,b){f.push.apply(f,A(m,"char","",i.whitespace&&b))}),f});function z(a){a=a||{};var i=a.key;return v(a.target||"[data-splitting]").map(function(s){var f=s["\u{1F34C}"];if(!a.force&&f)return f;f=s["\u{1F34C}"]={el:s};var m=I(a.by||d(s,"splitting")||k),b=D({},a);return g(m,function(y){if(y.split){var S=y.by,x=(i?"-"+i:"")+y.key,H=y.split(s,b,f);x&&M(s,x,H),f[S]=H,s.classList.add(S)}}),s.classList.add("splitting"),f})}function we(a){a=a||{};var i=a.target=u();return i.innerHTML=a.content,z(a),i.outerHTML}z.html=we,z.add=h;function G(a,i,s){var f=v(i.matching||a.children,a),m={};return g(f,function(b){var y=Math.round(b[s]);(m[y]||(m[y]=[])).push(b)}),Object.keys(m).map(Number).sort(ke).map(O(m))}function ke(a,i){return a-i}var Se=T("lines",[E],"line",function(a,i,s){return G(a,{matching:s[E]},"offsetTop")}),xe=T("items",N,"item",function(a,i){return v(i.matching||a.children,a)}),Te=T("rows",N,"row",function(a,i){return G(a,i,"offsetTop")}),Oe=T("cols",N,"col",function(a,i){return G(a,i,"offsetLeft")}),Ee=T("grid",["rows","cols"]),B="layout",Ce=T(B,N,N,function(a,i){var s=i.rows=+(i.rows||d(a,"rows")||1),f=i.columns=+(i.columns||d(a,"columns")||1);if(i.image=i.image||d(a,"image")||a.currentSrc||a.src,i.image){var m=v("img",a)[0];i.image=m&&(m.currentSrc||m.src)}i.image&&o(a,"background-image","url("+i.image+")");for(var b=s*f,y=[],S=u(N,"cell-grid");b--;){var x=u(S,"cell");u(x,"cell-inner"),y.push(x)}return c(a,S),y}),Le=T("cellRows",[B],"row",function(a,i,s){var f=i.rows,m=p(f);return g(s[B],function(b,y,S){m[Math.floor(y/(S.length/f))].push(b)}),m}),Ne=T("cellColumns",[B],"col",function(a,i,s){var f=i.columns,m=p(f);return g(s[B],function(b,y){m[y%f].push(b)}),m}),ze=T("cells",["cellRows","cellColumns"],"cell",function(a,i,s){return s[B]});return h(l),h(C),h(Se),h(xe),h(Te),h(Oe),h(Ee),h(Ce),h(Le),h(Ne),h(ze),z})})(ce);const Pe=ce.exports,je={view:async()=>`
